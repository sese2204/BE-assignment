...
서비스 요구사항에서 **구체적 정의가 부족한 부분**을 AI로 검토하여 사전에 식별했습니다.
...
AI API 호출처럼 응답이 느린 외부 통신에 대비해, `suspend` 함수와 `Flow`를 활용한 **논블로킹 비동기 처리**를 구현했습니다.
...
AI 클라이언트 레이어에 `suspend` 함수와 `Flow`를 적용해 논블로킹 구조로 설계했습니다.
